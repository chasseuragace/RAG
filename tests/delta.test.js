...
async function runDeltaTests() {
  ...
  const registry = new DocRegistry({ filePath: tmpRegistry });
  await registry.init();
  ...
  assert((await registry.get('doc1.md')).chunkCount === initialChunks, 'registry records the chunk count');
  ...
  assert((await registry.get('doc1.md')) === undefined, 'registry entry deleted');
  ...
  assert(!!(await registry.get('a.md')) && !!(await registry.get('b.md')), 'registry populated by full run');
